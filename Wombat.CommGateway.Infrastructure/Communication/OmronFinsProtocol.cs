...
namespace Wombat.CommGateway.Infrastructure.Communication
{
    ...
    /// Omron FINS协议实现
    /// </summary>
    public class OmronFinsProtocol : IProtocol
    {
        ...
        private byte _unitAddress;
        private string _lastError;

        public string Name => "OmronFins";
        ...
        public async Task InitializeAsync(Dictionary<string, string> configuration)
        {
            try
            {
                ...
                _networkAddress = byte.Parse(configuration["NetworkAddress"]);
                _nodeAddress = byte.Parse(configuration["NodeAddress"]);
                _unitAddress = byte.Parse(configuration["UnitAddress"]);
